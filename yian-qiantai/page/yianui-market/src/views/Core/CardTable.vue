<template>
  <div>
    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column type="selection" width="40" v-if="showBatchExport & showBatchDelete & showOperation"></el-table-column>
      ...
      <el-table-column :label="$t('action.operation')" width="300" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          <kt-button icon="fa fa-search" :label="$t('action.detail')" :perms="permsDetail" :size="size" type="primary" @click="handleDetails(scope.$index, scope.row)"/>    
          <kt-button icon="fa fa-edit" :label="$t('action.edit')" :perms="permsEdit" :size="size" type="primary" @click="handleEdit(scope.$index, scope.row)" />
    ...
    <div class="toolbar" style="padding:10px;">
      ...
      <kt-button :label="$t('action.batchExport')" :perms="permsExport" :size="size" type="primary" @click="handleBatchExport()" 
        :disabled="this.selections.length===0" style="float:left;" v-if="showBatchExport & showOperation"/>
...
export default {
  ...
  props: {
    ...
    permsExport: String, //查看详情标识
    permsDetail: String, //查看详情标识
    permsEdit: String,  // 编辑权限标识
    permsDelete: String,  // 删除权限标识
    //permsQuery:String,//查询
    size: { // 尺寸样式
      type: String,
      default: 'mini'
    },
    ...
    // 批量导出
      ...
      for(var i=0; i<idArray.length; i++) {
        params.push({'id':idArray[i]})
			...
			this.$emit('handleExport', {params:params})
    },
    // 删除
