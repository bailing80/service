  ...
  <div>
    ...
    <!--
       <template v-if="false"   slot-scope="scope" >
              <img :src="scope.row.pathqrcode" width="38px" height="38px">
          </template>  
    -->
     <!-- img v-for="(img,index) in imgs" v-preview="img.url"  :src="img.url" :alt="img.title"
                             :key="index"  preview-title-enable="true" -nav-enable="true" -->
      ...
      <el-table-column :label="$t('action.operation')" width="185" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope" >       
          ...
          <!-- kt-button icon="fa fa-edit" :label="$t('action.queryExpInfo')" :perms="permsQuery" :size="size" @click="handleQuery(scope.$index, scope.row)" / -->      
          <!-- img  :src="props.row.icon" alt="" style="width: 50px;height: 50px" -->         
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    //查询
		//handleQuery: function (index, row) {
		 // this.$emit('handleQuery', {index:index, row:row})
    //},
